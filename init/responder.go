...
// followers' SNTP queries from their own disciplined clocks. It is a
// responder, not a proxy: no query is ever forwarded upstream. Each
// stratum serves the clock it keeps and advertises which reference
// that clock descends from. This is how the real NTP hierarchy
// works, and it is what makes a leader self-sufficient once synced:
// followers can boot, join, and stay disciplined with no internet
// access at all.
//
// The packet is 48 bytes, fixed layout, big-endian. Like the GPT
// header, it is small enough to build by hand and documented well
// enough (RFC 5905) to build correctly. The reply carries four
...
// have no port open unless something is expected to connect to it.
// This is also the machine plane's named candidate for promotion to
...
// The mode field, three bits of byte zero: what kind of packet this
// is. A client asks with mode 3; a server answers with mode 4. Every
// other mode belongs to NTP's symmetric and broadcast machinery,
// which liken doesn't implement; those packets get no reply.
const (
	modeClient = 3
	modeServer = 4
)

// The leap indicator, two bits of byte zero. Its primary purpose is
// announcing leap seconds, but it also carries the alarm value 3,
...
// seconds since 1900, then 32 bits of binary fraction, where each
// unit is 1/2^32 of a second, about 233 picoseconds. That is far
// finer than anything here measures.
...
// clock, from the same state status.time reports. The wire's
// vocabulary differs from status's in one spot: a server that wants
// time and hasn't gotten it yet answers stratum 0 with the "INIT"
// kiss code (the protocol's "don't use me, ask later"), where status
// says stratum 16. Both mean unsynchronized; one describes this
// machine, the other instructs a client.
func (c *clock) advertise() (stratum, leap byte, lastSync *timeSync) {
	c.mu.Lock()
	defer c.mu.Unlock()
	switch {
	case len(c.sources) == 0:
		// Deliberately free-running: serve at the local-clock
		// stratum without the alarm. A fleet with no upstreams still
		// needs one shared reference, and this machine's clock is it.
...
// anything that isn't a plausible client request. Garbage gets no
// reply rather than an error: port 123 on an open network receives
// all sorts of packets, and the cheapest response is none.
func respondTo(request []byte, clk *clock, now time.Time) []byte {
	...
	// gets an SNTPv3 answer.
	response[0] = leap<<6 | version<<3 | modeServer
	response[1] = stratum
	response[2] = request[2] // the client's poll interval, echoed by convention
	// Precision is a signed log2: -20 claims ~1µs, a realistic claim
	// for a clock read from the kernel.
	...
	// local clock, or, for stratum 0, the kiss code: an ASCII
	...
	// back verbatim as the originate timestamp (t1, which is how the
	// client matches replies to requests and rejects spoofed strays),
	// and t2/t3 bracket our handling. One clock reading serves both:
	// the nanoseconds this function takes are noise next to the
...
// context ends or the socket fails. The context's goroutine closes
// the socket to unblock the read: a blocked ReadFrom has no other
...
func answerTimeQueries(ctx context.Context, pc net.PacketConn, clk *clock) error {
	...
	for {
		...
		if response := respondTo(buffer[:n], clk, time.Now()); response != nil {
			...
			// doesn't guarantee delivery, and neither does this
			// responder.
...
// failed socket) hands the retry to the plane's backoff rather than
// looping here.
