...
// and to files on the machine. None of that is visible to `kubectl
// logs` or to any log stack someone might run, because a collector
// cannot tail a serial port. Each relay reads one stream at its
// source and sends it out again, line by line, on its own stdout.
// Because each relay runs as a pod, its stdout is a pod log: the
// Kubernetes API serves it, RBAC controls access to it, and any
// in-cluster collector reads it with no host privileges. The OS's
// log interface is `kubectl logs`, the same API as everything else.
//
// One binary carries all four relays, chosen by an argument verb
// (the multi-call pattern, the same pattern init uses to re-exec
...
//
// The kernel and init share /dev/kmsg. init writes its lines there so
// they interleave with the kernel's lines in true order. The
// facility field is what splits them back apart, so each pod carries
// exactly one program's records.
//
...
// top of this; restarting from a durable position is the retry
// logic.
package main
...
// cursors live. These paths are fixed facts of the DaemonSet's
// mounts. Tests point a relay at a temporary directory through its
// parameters instead.
const (
	cursorDir         = "/cursor"
...
func main() {
	// The version goes to stderr, because stdout carries only data:
	// the relay never writes anything to stdout except an envelope.
