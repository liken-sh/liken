...
* The proprietary userspace driver (libcuda, nvidia-smi),
  ...
  must read closely.
* The container toolkit, which k3s turns into a containerd runtime by
  itself.
...
risk stays off the boot path, because a wrong build breaks GPU
workloads and the base OS boots regardless.
...
amdgpu. One measurement settles amdgpu: its 108 MiB of firmware
compresses to 22 MiB in the squashfs, so removal saves almost nothing
and it would cost the console on every machine with AMD integrated
graphics. The radeon driver does not cover those machines. radeon
...
shape later, but the display firmware stays stock.
