...
// first. The boot roles lead so the firmware can find them: the BIOS
// boot code comes earliest of all. machineState comes before the
// data roles, because a boot must find that partition before it has
// read any spec.
