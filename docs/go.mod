// The docs domain is its own Go module, apart from the repository's
// root module, for one reason: Hugo. The site builds with Hugo as a
// pinned tool dependency, and Hugo's dependency graph is hundreds of
// modules that nothing else in the repository reads. A nested module
// keeps that graph in this go.sum, out of the root module's.
//
// The standard Hugo build is pure Go, so `go tool hugo` compiles and
// runs it from the module cache. The tool directive pins the version
