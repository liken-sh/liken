...
#
# A container image is not an exotic artifact: it is a tarball of
...
# *index* is the front door that names the manifest. Every blob is
# stored under the SHA-256 of its bytes and referred to only by that
# digest — an image is a little content-addressed database, which is
...
# directly — so the format holds no mysteries. The result lands in the
# initramfs at /var/lib/rancher/k3s/agent/images/, where k3s imports
# every archive it finds into containerd at startup. That is liken's
# entire image distribution story: no registry, no pull, no network —
# whoever holds the OS image holds every byte the machine will run.
...
# image and no shell to name; the Entrypoint is the whole story.
